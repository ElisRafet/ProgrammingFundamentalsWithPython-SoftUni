...
def swap_lesson(list_of_lessons, first_lesson, second_lesson):
    if first_lesson in list_of_lessons and second_lesson in list_of_lessons:
        ...
        if first_lesson_has_exercise and second_lesson_has_exercise:
            list_of_lessons[first_position + 1], list_of_lessons[second_position + 1] = list_of_lessons[second_position + 1], list_of_lessons[first_lesson + 1]
...
while len(command) > 1:
    ...
    if action == 'Add':
        ...
        index = command[2]
        lesson = insert_lesson(lessons, lesson_title, index)
    elif action == 'Remove':
        lesson = remove_lesson(lessons, lesson_title)
    elif action == 'Swap':
        next_lesson_title = command[2]
        lesson = swap_lesson(lessons, lesson_title, next_lesson_title)
    elif action == 'Exercise':
        lesson = exercise(lessons, lesson_title)
        command = input().split(':')

for index, lesson_or_exerxise in enumerate(lessons):
    print(f'{index + 1}.{lesson_or_exerxise}')
